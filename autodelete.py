...
def get_tasks_to_delete(tasks: List[TaskData], days_threshold: int) -> List[TaskData]:
    ...
    logger.info(f"Cutoff date for deletion: {cutoff_date.strftime('%Y-%m-%d %H:%M:%S UTC')}")
    ...
    for task in tasks:
        ...
        if task_modified_date.tzinfo is None:
            task_modified_date = task_modified_date.replace(tzinfo=datetime.timezone.utc)
        
        if task_modified_date < cutoff_date:
            days_old = (cutoff_date - task_modified_date).days
            logger.debug(f"Task '{task.text}' qualifies for deletion (modified {days_old} days ago)")
...
@click.option('--username', help='CalDAV username (or set CALDAV_TASKS_API_USERNAME env var)')
@click.option('--password', help='CalDAV password (or set CALDAV_TASKS_API_PASSWORD env var)')
@click.option('--nextcloud-mode/--no-nextcloud-mode', default=True, 
              help='Enable Nextcloud CalDAV path adjustment (default: enabled)')
@click.option('--debug/--no-debug', default=False, help='Enable debug mode with detailed logging')
@click.option('--ssl-verify/--no-ssl-verify', default=True, help='Verify SSL certificates (default: enabled)')
@click.option('--dry-run/--execute', default=True, 
              help='Show what would be deleted without actually deleting (default: dry-run)')
def main(list_uid: str, days_threshold: int, url: Optional[str], username: Optional[str], 
         password: Optional[str], nextcloud_mode: bool, debug: bool, ssl_verify: bool, dry_run: bool):
    """
    ...
    logger.info(f"Mode: {'DRY RUN (safe preview)' if dry_run else 'EXECUTE (will delete tasks)'}")
    ...
    try:
        ...
        logger.info(f"Found {len(tasks_to_delete)} completed tasks older than {days_threshold} days")
        ...
        for i, task in enumerate(tasks_to_delete, 1):
            ...
            days_old = (datetime.datetime.now(datetime.timezone.utc) - modified_date).days
            
            logger.info(f"{i:3}. '{task.text[:60]}{'...' if len(task.text) > 60 else ''}'")
            logger.info(f"     Last modified: {modified_date.strftime('%Y-%m-%d %H:%M:%S UTC')} ({days_old} days ago)")
