...
| Update Task             | ❌                                        | Partial (Not directly tested as a dedicated API method) | No high-level `update_task()` method. Updates require fetching a `caldav.Todo` object, modifying its `data` with `TaskData.to_ical()`, and calling `save()`. This is an advanced operation. |
