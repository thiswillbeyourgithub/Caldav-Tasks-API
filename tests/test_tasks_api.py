import pytest
from caldav_tasks_api import (
    TasksAPI,
    TaskData,
    TaskListData,
)  # Assuming tasks_api is importable
import uuid
import subprocess
...
def test_fetch_task_lists(tasks_api_instance: TasksAPI):
    ...
    if not api.task_lists:
        print(
            "Warning: No task lists found on the server. Test might not be comprehensive."
        )

    for task_list in api.task_lists:
        assert isinstance(
            task_list, TaskListData
        ), "Each item in task_lists should be TaskListData."
        assert task_list.uid, f"Task list '{task_list.name}' should have a UID."
        assert (
            task_list.name is not None
        ), f"Task list UID '{task_list.uid}' should have a name."
...
def test_find_tasks_in_lists(tasks_api_instance: TasksAPI):
    ...
    for task_list in api.task_lists:
        ...
        ), f"Tasks attribute for list '{task_list.name}' should be a list."
        if task_list.tasks:
            found_any_task = True
...
def test_create_and_delete_task(tasks_api_instance: TasksAPI, test_list_name: str):
    ...
    if not target_list:
        pytest.skip(
            f"Test list '{test_list_name}' not found on the server. Skipping create/delete test."
        )
    ...
    assert (
        target_list_after_add is not None
    ), "Target list not found after reloading for add."

    assert (
        len(target_list_after_add.tasks) == original_task_count + 1
    ...
    assert (
        target_list_after_delete is not None
    ), "Target list not found after reloading for delete."

    assert (
        len(target_list_after_delete.tasks) == original_task_count
    ), "Task count should revert to original after deleting."
    ...
    for task in target_list_after_delete.tasks:
        if task.uid == task_uid_to_delete:
            ...
            break
    assert (
        task_should_be_gone is None
    ), f"Deleted task UID '{task_uid_to_delete}' still found in list."
...
def test_create_single_task(tasks_api_instance: TasksAPI, test_list_name: str):
    ...
    if not target_list:
        pytest.skip(
            f"Test list '{test_list_name}' not found on the server. Skipping create task test."
        )
    ...
    assert (
        target_list_before_add is not None
    ...
    created_task_data = api.add_task(
        task_data=task_to_create, list_uid=target_list_before_add.uid
    )
    assert (
        created_task_data is not None
    ), "add_task should return the created task data."
    ...
    api.load_remote_data()  # Reload all data to ensure we see the server state
    target_list_after_add = api.get_task_list_by_uid(
        target_list_before_add.uid
    )  # Use original UID
    assert (
        target_list_after_add is not None
    ), "Target list not found after reloading for add."
...
def test_create_and_rename_task(tasks_api_instance: TasksAPI, test_list_name: str):
    ...
    if not target_list:
        pytest.skip(
            f"Test list '{test_list_name}' not found on the server. Skipping create/rename test."
        )
    ...
    assert (
        target_list_after_rename is not None
    ), "Target list not found after reloading for rename verification."
...
def test_task_to_dict(tasks_api_instance: TasksAPI, test_list_name: str):
    ...
    if not target_list:
        pytest.skip(
            f"Test list '{test_list_name}' not found on the server. Skipping to_dict test."
        )

    if not target_list.tasks:
        print(
            f"Warning: No tasks found in the test list '{test_list_name}'. Test test_task_to_dict might not be comprehensive."
        )
        return  # Nothing to test if there are no tasks

    print(
        f"Found {len(target_list.tasks)} tasks in list '{test_list_name}' for to_dict testing."
    )
    for task in target_list.tasks:
        ...
        assert (
            task_dict["list_uid"] == task.list_uid
        ), "Task dictionary 'list_uid' should match task.list_uid."
        ...
        assert (
            "x_properties" in task_dict
        ), "Task dictionary should contain 'x_properties'."
        assert isinstance(
            task_dict["x_properties"], dict
        ), "'x_properties' in task_dict should be a dict."
...
def test_cli_show_summary_json_runs_successfully(caldav_credentials):
    ...
    try:
        ...
        pytest.fail(
            f"CLI command failed: `python -m caldav_tasks_api` could not be found. Ensure the package is installed correctly, e.g., `pip install -e .`"
        )


def test_add_task_in_read_only_mode(
    read_only_tasks_api_instance: TasksAPI, test_list_name: str
):
    ...
    if api.task_lists:
        ...
        if (
            not target_list_uid
        ):  # Fallback to the first list if test_list_name not found
...
def test_delete_task_in_read_only_mode(
    read_only_tasks_api_instance: TasksAPI, test_list_name: str
):
...
def test_update_task_in_read_only_mode(
    read_only_tasks_api_instance: TasksAPI, test_list_name: str
):
    ...
    if api.task_lists:
        ...
        if test_list_found and test_list_found.tasks:
            ...
            existing_task_uid = test_list_found.tasks[
                0
            ].uid  # Use the first task from the test list
    ...
    if not target_list_uid or not existing_task_uid:
        ...
        existing_task_uid = (
            existing_task_uid or f"dummy-task-uid-for-update-{uuid.uuid4()}"
        )
...
def test_create_update_xprop_delete_task(
    ...
    if not target_list:
        pytest.skip(
            f"Test list '{test_list_name}' not found on the server. Skipping test."
        )
    ...
    assert (
        x_prop_name in found_task.x_properties
    ), f"X property {x_prop_name} should exist in task. Available props: {list(x_props.keys())}"
    ...
    for k in x_props.keys():
        if k.lower() == x_prop_name.lower() or (
            ...
            k.split("-", 2)[0:2] == x_prop_name.split("-", 2)[0:2]
            and k.split("-", 2)[2].lower() == x_prop_name.split("-", 2)[2].lower()
        ):
            actual_key = k
            break
    ...
    ), f"Failed to find any key matching {x_prop_name} in {list(x_props.keys())}"
    assert (
        x_props[actual_key] == x_prop_value
    ), f"X property value should be {x_prop_value}, got {x_props.get(actual_key)}"
    ...
    for task in task_list.tasks:
        assert (
            task.uid != created_task.uid
        ), f"Task UID {created_task.uid} should not exist after deletion"
...
def test_task_ical_roundtrip():
    ...
    roundtrip_task = TaskData.from_ical(
        ical=ical_string, list_uid=original_task.list_uid
    )
    ...
    assert (
        roundtrip_task.start_date == original_task.start_date
    ), "DTSTART should be preserved"
    assert (
        roundtrip_task.priority == original_task.priority
    ), "PRIORITY should be preserved"
    assert set(roundtrip_task.tags) == set(
        original_task.tags
    ), "CATEGORIES (tags) should be preserved"
    assert (
        roundtrip_task.percent_complete == original_task.percent_complete
    ), "PERCENT-COMPLETE should be preserved"

    # Check X-properties preservation
    for key, value in original_task.x_properties.items():
        assert (
            key in roundtrip_task.x_properties.get_raw_properties()
        ), f"X-property {key} should be preserved"
        assert (
            roundtrip_task.x_properties[key] == value
        ), f"X-property {key} value should be preserved"
    ...
    assert (
        second_ical_string == ical_string
    ), "Double conversion (to_ical→from_ical→to_ical) should produce the same output"

    print(
        "Successfully verified TaskData.to_ical() → from_ical() → to_ical() roundtrip"
    )
...
def test_task_parent_child_relationships(
    ...
    if not target_list:
        pytest.skip(
            f"Test list '{test_list_name}' not found on the server. Skipping parent-child relationship test."
        )
    ...
    print(
        "Reloading all remote data to ensure API instance has fresh data and correct references."
    )
    api.load_remote_data()
    ...
    assert (
        retrieved_parent is not None
    ), f"Parent task (UID: {created_parent_task.uid}) should be found after reload."
    assert (
        retrieved_child is not None
    ), f"Child task (UID: {created_child_task.uid}) should be found after reload."

    # --- Test child's parent relationship ---
    print(f"Verifying child task (UID: {retrieved_child.uid}) parent relationship...")
    assert (
        retrieved_child.parent_task is not None
    ), "Child's parent_task property should resolve to a TaskData instance."
    ...
    assert (
        retrieved_parent.child_tasks
    ), "Parent's child_tasks property should return a non-empty list."

    found_child_in_parent_list = next(
        (t for t in retrieved_parent.child_tasks if t.uid == retrieved_child.uid), None
    )
    ...
    assert (
        found_child_in_parent_list.uid == retrieved_child.uid
    ...
    assert (
        target_list_after_cleanup is not None
    ), "Target list should still exist after cleanup."

    task_should_be_gone_child = api.get_task_by_global_uid(created_child_task.uid)
    assert (
        task_should_be_gone_child is None
    ), f"Child task UID '{created_child_task.uid}' should not be found after deletion."

    task_should_be_gone_parent = api.get_task_by_global_uid(created_parent_task.uid)
    assert (
        task_should_be_gone_parent is None
    ), f"Parent task UID '{created_parent_task.uid}' should not be found after deletion."
